...
export default function Map() {
  ...
  const accent = isSharedTrip
    ...
    : { border: 'border-sky/40', ring: 'shadow-[0_0_60px_-25px_rgba(20,184,166,0.55)]', label: 'text-sky', hover: 'hover:border-sky/70', bgTint: 'bg-teal/5' }

  return (
    <main
      className={[
        'fixed inset-0 bg-cloud text-mist',
    ...
    >
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-30">
        <div className="pointer-events-auto mx-auto flex max-w-7xl items-center justify-between rounded-b-2xl bg-white/85 px-6 py-4 shadow-card backdrop-blur">
          ...
          <nav className="flex items-center gap-1 sm:gap-2">
            ...
            <NavLink to="/map" className={navClass} end>Map</NavLink>
      ...
      <div className="pointer-events-none absolute inset-x-0 top-24 z-30 flex justify-center px-4">
        <div
          className={[
            'pointer-events-auto flex items-center gap-3 rounded-full border bg-white/95 px-4 py-2 shadow-card backdrop-blur',
            accent.border,
          ].join(' ')}
        >
          <span className={['text-[10px] font-semibold tracking-[0.32em]', accent.label].join(' ')}>
          ...
          {isSharedTrip && invitedEmail ? (
            <>
              <span className="h-3 w-px bg-slate-200" />
              <span className="text-xs text-ink-muted">with {invitedEmail}</span>
            </>
          ) : null}
          {!tripLoading && (isSharedTrip ? tripId : true) ? (
            <>
              ...
              <button
                ...
                className={[
                  'rounded-full border px-3 py-1 text-[11px] tracking-wide text-ink transition-colors',
                  accent.border,
                  accent.hover,
                  accent.bgTint,
                ].join(' ')}
      ...
      {isSharedTrip && tripId ? (
        <div className="pointer-events-none absolute inset-x-0 top-40 z-30 flex justify-center px-4">
      ...
      {tripLoading ? (
        <div className="pointer-events-none absolute inset-x-0 top-36 z-30 mx-auto max-w-xs rounded-full border border-slate-200 bg-white/95 px-4 py-2 text-center text-[11px] tracking-[0.32em] text-muted backdrop-blur">
          LOADING YOUR PINS…
        </div>
      ) : null}

      {tripError ? (
        <div className="absolute inset-x-0 top-36 z-30 mx-auto max-w-md rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-center text-sm text-red-700">
          {tripError}
        </div>
      ) : null}
      ...
      <AnimatePresence>
        {tooltip ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="pointer-events-none fixed z-40 rounded-md border border-slate-200 bg-white/95 px-3 py-1.5 text-xs text-mist shadow-soft backdrop-blur-sm"
...
function Legend() {
  ...
  return (
    <div className="pointer-events-none absolute bottom-6 left-6 z-20 rounded-2xl border border-slate-100 bg-white/95 px-4 py-3 shadow-lift backdrop-blur">
      <div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.32em] text-ink-muted">
        Legend
      </div>
      <div className="flex flex-col gap-1.5">
        {items.map((item) => (
          <div key={item.label} className="flex items-center gap-2 text-xs font-medium text-ink">
            <span
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between border-b border-slate-200 px-6 py-6">
        <div>
          <div className="text-[11px] tracking-[0.32em] text-sky">COUNTRY</div>
          <div className="mt-2 flex items-center gap-3">
            <span className="text-3xl leading-none">{flagEmoji(country.code)}</span>
            <h2 className="font-display text-2xl text-ink">{country.name}</h2>
          </div>
        </div>
        <button
          type="button"
          onClick={onClose}
          className="rounded-full border border-slate-200 px-3 py-1 text-xs text-mist/70 transition-colors hover:border-sky/40 hover:text-ink"
...
function SidebarTabs({ countryId, status }) {
  ...
  return (
    <div>
      <div className="mb-3 flex gap-1 rounded-full border border-slate-200 bg-white/95 p-1">
        <button
          type="button"
...
function MemoriesSection({ countryId }) {
  ...
  if (!countryId) {
    return (
      <p className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-mist/60">
    ...
    <div className="space-y-4">
      ...
      >
        <input
          ...
          className="block w-full text-xs text-mist/70 file:mr-3 file:cursor-pointer file:rounded-full file:border-0 file:bg-sky/15 file:px-3 file:py-1.5 file:text-xs file:text-sky hover:file:bg-teal/30"
        />
        <textarea
          value={note}
              ...
              >
                ...
                {m.user_id === user?.id ? (
                  <button
                    ...
                    className="shrink-0 rounded-md border border-slate-200 px-2 py-1 text-[11px] text-mist/60 transition-colors hover:border-red-400/40 hover:text-red-700"
    ...
    <div>
      <div className="mb-3 flex items-center justify-between">
        ...
        {countryId ? (
          <button
            ...
            className="text-xs text-sky transition-colors hover:text-sky-deep"
          >
            {showForm ? 'Cancel' : '+ Add'}
          </button>
        ) : null}
      ...
      {!countryId ? (
        ...
        <>
          ...
          {listError ? (
            <p className="mt-2 text-xs text-red-700">{listError}</p>
          ) : null}
  ...
  return (
    <button
      ...
      className={[
        ...
        active
          ? 'border-white/60 bg-white/5 text-ink'
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="flex h-[70vh] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-sky/20 bg-white shadow-[0_0_80px_-20px_rgba(20,184,166,0.35)]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between border-b border-slate-200 px-5 py-4">
          <div>
            <div className="text-[11px] tracking-[0.32em] text-sky">ADD COUNTRY</div>
            <h2 className="mt-1 font-display text-xl text-ink">Pick a destination</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          {loading ? (
            ...
            <p className="mx-3 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
              {error}
            </p>
          ) : filtered.length === 0 ? (
                ...
                return (
                  <li key={c.code}>
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left text-sm text-mist transition-colors hover:bg-teal/5 hover:text-ink"
                    >
