...
function StatCard({ value, label, tone, loading, icon }) {
  const toneClasses = {
    sky: {
      ...
      chip: 'bg-white/70 text-sky',
      text: 'text-sky',
    },
    forest: {
      card: 'bg-[#F0FDF4] border-[#BBF7D0]',
      chip: 'bg-white/70 text-forest',
      text: 'text-forest',
    },
    sand: {
      card: 'bg-[#FFFBEB] border-[#FDE68A]',
      chip: 'bg-white/70 text-[#B45309]',
      text: 'text-[#B45309]',
    },
  }[tone]
  ...
  return (
    <div
      className={[
        'rounded-2xl border p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md',
        toneClasses.card,
      ].join(' ')}
    >
      <div className="flex items-center justify-between">
        <div className={['flex h-10 w-10 items-center justify-center rounded-xl', toneClasses.chip].join(' ')}>
          {icon}
        </div>
...
function LatestTripCard({ trip, counts, onOpen }) {
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="group block w-full rounded-2xl border-l-4 border-l-sky border border-slate-200 bg-white p-8 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"
    >
...
function QuickCard({ to, title, body, tone }) {
  const toneClasses = {
    sky: { card: 'bg-[#EFF6FF] border-[#BFDBFE]', chip: 'bg-white/70 text-sky' },
    forest: { card: 'bg-[#F0FDF4] border-[#BBF7D0]', chip: 'bg-white/70 text-forest' },
    sand: { card: 'bg-[#FFFBEB] border-[#FDE68A]', chip: 'bg-white/70 text-[#B45309]' },
  }[tone]
  ...
  return (
    <Link
      to={to}
      className={[
        'group rounded-2xl border p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md',
        toneClasses.card,
      ].join(' ')}
    >
      <div className={['mb-4 inline-flex h-10 w-10 items-center justify-center rounded-xl', toneClasses.chip].join(' ')}>
        <ArrowIcon />
      </div>
