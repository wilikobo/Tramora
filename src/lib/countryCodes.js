...
// world-atlas topojson uses numeric ids on each geography, so we map back to
// alpha-2 codes only where we need them (currently: flag emoji rendering).
...
export function numericToAlpha2(id) {
...
export function flagEmoji(id) {
  ...
  try {
    return String.fromCodePoint(
      alpha2.charCodeAt(0) + base,
      alpha2.charCodeAt(1) + base,
    )
  } catch {
    return '🌍'
  }
